...
test('requires explicit release identity and version history inputs', () => {
  assert.throws(() => packagingConfig([]), /existing Play/);
  assert.throws(() => packagingConfig(['--package-id', 'com.borsabi.trader']), /version-code/);
  assert.deepEqual(packagingConfig(['--package-id', 'com.borsabi.trader', '--version-code', '7', '--version-name', '1.2.0']),
    { preview: false, packageId: 'com.borsabi.trader', versionCode: 7, versionName: '1.2.0' });
});
test('keeps the preview identity separate and rejects code injection and malformed versions', () => {
  ...
  for (const code of ['0', '-1', '1.5', '2100000001', 'NaN']) {
    assert.throws(() => packagingConfig(['--package-id', 'com.borsabi.trader', '--version-code', code, '--version-name', '1.0.0']));
  }
});
