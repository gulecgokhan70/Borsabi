    ...
    for (const bot of bots) {
      ...
      try {
        if ((bot.config as { mode?: string }).mode === 'auto-v2') {
          ...
          const result = autoStep(state as AutoState, c, await autoObservations(c), Date.now());
      ...
      await persistBot(prisma, bot.id, bot.version, state, message, events.filter(e => e.action !== 'WAIT' || e.reason !== bot.message));
      processed++;
    }
