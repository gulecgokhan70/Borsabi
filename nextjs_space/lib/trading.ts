...
export async function executeTrade(db: PrismaClient, userId: string, input: TradeInput, executionPrice: number, fx?: FxQuote) {
    ...
    try {
      return await db.$transaction(async tx => {
        ...
        if (type === 'BUY') {
          ...
          return { success: true, message: `${quantity} adet ${symbol} alındı`, warnings, price, priceTry, fxRate };
        }
        ...
        return { success: true, message: `${quantity} adet ${symbol} satıldı. K/Z: ${sale.pnl.toFixed(2)} TL`, warnings, pnl: sale.pnl, price, priceTry, fxRate };
