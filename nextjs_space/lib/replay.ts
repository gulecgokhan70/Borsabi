...
type ReplayTrade = { side: 'BUY' | 'SELL'; quantity: number; price: number; commission: number; time: string; note: string; pnl?: number };
export type ReplayState = { symbol: string; currency: 'USD' | 'TRY'; bars: ReplayBar[]; cursor: number; cash: number; initial: number;
  quantity: number; cost: number; fees: number; trades: ReplayTrade[]; finished: boolean; source: string; fetchedAt: string };
...
export function createReplay(symbol: string, bars: ReplayBar[], fetchedAt = new Date().toISOString()): ReplayState {
  ...
  return { symbol, currency, bars, cursor: 29, cash: initial, initial, quantity: 0, cost: 0, fees: 0, trades: [], finished: false, source: 'Yahoo Finance — geçmiş 5 dakikalık mumlar', fetchedAt };
}
export function applyReplay(input: ReplayState, action: ReplayAction): ReplayState {
  ...
  const bar = s.bars[s.cursor], total = quantity * bar.close, commission = total * 0.002;
  ...
  s.fees += commission;
  s.trades.push({ side: action.action, quantity, price: bar.close, commission, time: bar.time, note: action.note, pnl });
  return s;
}
...
export function replayView(s: ReplayState) {
  ...
  return { ...s, bars: s.bars.slice(0, s.cursor + 1), stepsRemaining: s.bars.length - s.cursor - 1, equity,
    ...
    assessment: s.finished ? [
      `${s.trades.length} işlem yaptınız; toplam komisyon ${s.fees.toFixed(2)} ${s.currency}.`,
      `Gözlenen son fiyatta toplam sonuç ${(equity - s.initial).toFixed(2)} ${s.currency}.`,
