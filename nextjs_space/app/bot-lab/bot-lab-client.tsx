      ...
      {bots.map(b => {
        ...
        return <section key={b.id} className="glass-card rounded-2xl p-5 space-y-4">
          ...
          {auto && <>
            ...
            {Object.entries(b.state.holdings).map(([symbol, h]) => <div key={symbol} className="rounded-xl border p-3 text-sm space-y-1"><p className="font-semibold"><Link href={assetHref(symbol)} prefetch={false} className="text-blue-600 underline">{assetLabel(symbol)}</Link> · {h.quantity.toLocaleString('tr-TR', { maximumFractionDigits: 8 })} adet</p><p>Alış {money(h.entry)} · Son fiyat {money(h.mark)}</p><p>Zarar sınırı {money(h.entry * (1 - b.config.stopLoss))} · Hedef {money(h.entry * (1 + b.config.takeProfit))}</p><p className="text-muted-foreground">Fiyat zamanı: {date(h.quoteTime)}</p></div>)}
