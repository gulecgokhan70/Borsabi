...
export function ResumeCard({ accountId }: { accountId: string }) {
  ...
  if (state.hidden) return <button className="min-h-[44px] text-sm text-blue-500 underline" onClick={() => updateResume(accountId, { hidden: false })}>Devam kartını göster</button>;
